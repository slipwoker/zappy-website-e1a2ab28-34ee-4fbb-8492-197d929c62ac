document.addEventListener('DOMContentLoaded', function () {
      ...
      if (field.hasAttribute('required') && value === '') {
        ...
        if (type === 'email' && !isValidEmail(value)) {
          ...
          if (value.length < minLength) {
            errorMessage = getErrorMessage(field, 'minlength', minLength);
            isValid = false;
          }
        } else if (field.hasAttribute('maxlength')) {
          const maxLength = parseInt(field.getAttribute('maxlength'), 10);
          if (value.length > maxLength) {
            errorMessage = getErrorMessage(field, 'maxlength', maxLength);
            isValid = false;
          }
        }
      }
  ...
  function clearFormErrors(form) {
    ...
    form.querySelectorAll('.form-success-message, .form-global-error').forEach(function (el) {
      el.remove();
    });
  }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "הרב רפאל מנת משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: 'e1a2ab28-34ee-4fbb-8492-197d929c62ac',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || 'e1a2ab28-34ee-4fbb-8492-197d929c62ac';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'e1a2ab28-34ee-4fbb-8492-197d929c62ac';
